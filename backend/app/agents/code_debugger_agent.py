...
SYSTEM_PROMPT = """Given a piece of code and an error message, identify and correct the error."""


class CodeDebuggerAgent(BaseAgent):
    """Debugs code for simple tasks"""
    ...
    async def execute(
        ...
        logger.info("[CodeDebuggerAgent] processing query (len=%d)", len(query))
        self.ensure_loaded()
        context = context or {}
        ...
        with Timer() as t:
            ...
            try:
                ...
                logger.exception("[CodeDebuggerAgent] generation failed")
                return AgentResult(
