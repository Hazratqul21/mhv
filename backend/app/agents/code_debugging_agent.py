...
SYSTEM_PROMPT = """Given a piece of code and an error message, identify the issue and suggest a solution."""


class CodeDebuggingAgent(BaseAgent):
    ...
    async def execute(
        ...
        logger.info("[CodeDebuggingAgent] processing query (len=%d)", len(query))
        self.ensure_loaded()
        context = context or {}
        ...
        with Timer() as t:
            ...
            try:
                ...
                logger.exception("[CodeDebuggingAgent] generation failed")
                return AgentResult(
