...
SYSTEM_PROMPT = """Given code provided by the user, execute it and return the output."""


class CodeExecutionAgent(BaseAgent):
    """Executes code provided by the user"""
    ...
    async def execute(
        ...
        logger.info("[CodeExecutionAgent] processing query (len=%d)", len(query))
        self.ensure_loaded()
        context = context or {}
        ...
        with Timer() as t:
            ...
            try:
                ...
                logger.exception("[CodeExecutionAgent] generation failed")
                return AgentResult(
