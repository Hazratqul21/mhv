...
SYSTEM_PROMPT = """Given a text input, generate a code snippet in a programming language that prints 'hello'."""


class CodeForTextGenerationAgent(BaseAgent):
    """Generates code based on text input"""

    def __init__(self, llm_engine: LLMEngine) -> None:
        settings = get_settings()
        super().__init__(
            name="code_for_text_generation",
    ...
    async def execute(
        ...
        logger.info("[CodeForTextGenerationAgent] processing query (len=%d)", len(query))
        self.ensure_loaded()
        context = context or {}
        ...
        with Timer() as t:
            ...
            try:
                ...
                logger.exception("[CodeForTextGenerationAgent] generation failed")
                return AgentResult(
