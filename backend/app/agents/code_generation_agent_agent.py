...
SYSTEM_PROMPT = """You are a code generation agent. Your task is to generate code that prints the string 'hello'."""


class CodeGenerationAgentAgent(BaseAgent):
    """Generates code for simple tasks such as printing messages"""
    ...
    async def execute(
        ...
        logger.info("[CodeGenerationAgentAgent] processing query (len=%d)", len(query))
        self.ensure_loaded()
        context = context or {}
        ...
        with Timer() as t:
            ...
            try:
                ...
                logger.exception("[CodeGenerationAgentAgent] generation failed")
                return AgentResult(
