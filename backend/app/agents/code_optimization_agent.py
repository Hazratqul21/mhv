...
SYSTEM_PROMPT = """Given a piece of code, suggest optimizations to improve its efficiency."""
...
class CodeOptimizationAgent(BaseAgent):
    ...
    async def execute(
        ...
        logger.info("[CodeOptimizationAgent] processing query (len=%d)", len(query))
        self.ensure_loaded()
        context = context or {}
        ...
        with Timer() as t:
            ...
            try:
                ...
                logger.exception("[CodeOptimizationAgent] generation failed")
                return AgentResult(
