...
SYSTEM_PROMPT = """You are a code prompt agent. Your job is to write code that prints 'hello'."""


class CodePromptAgentAgent(BaseAgent):
    """An agent that can write code to perform a specific task, in this case, print 'hello'."""

    def __init__(self, llm_engine: LLMEngine) -> None:
        settings = get_settings()
        super().__init__(
            name="code_prompt_agent",
    ...
    async def execute(
        ...
        logger.info("[CodePromptAgentAgent] processing query (len=%d)", len(query))
        self.ensure_loaded()
        context = context or {}
        ...
        with Timer() as t:
            ...
            try:
                ...
                logger.exception("[CodePromptAgentAgent] generation failed")
                return AgentResult(
