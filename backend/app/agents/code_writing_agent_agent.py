...
SYSTEM_PROMPT = """Given a simple programming task, write the code to perform it"""


class CodeWritingAgentAgent(BaseAgent):
    """Writes code for simple tasks"""
    ...
    async def execute(
        ...
        logger.info("[CodeWritingAgentAgent] processing query (len=%d)", len(query))
        self.ensure_loaded()
        context = context or {}
        ...
        with Timer() as t:
            ...
            try:
                ...
                logger.exception("[CodeWritingAgentAgent] generation failed")
                return AgentResult(
