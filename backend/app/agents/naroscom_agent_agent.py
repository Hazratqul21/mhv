...
SYSTEM_PROMPT = """Respond to queries related to NAROSCOM"""


class NaroscomAgentAgent(BaseAgent):
    """Handles queries related to NAROSCOM"""
    ...
    async def execute(
        ...
        logger.info("[NaroscomAgentAgent] processing query (len=%d)", len(query))
        self.ensure_loaded()
        context = context or {}
        ...
        with Timer() as t:
            ...
            try:
                ...
                logger.exception("[NaroscomAgentAgent] generation failed")
                return AgentResult(
